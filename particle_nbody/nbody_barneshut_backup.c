...
void nbodybarneshut (particle_t * array, int nbr_particles, int nbr_iterations, int psize, int prank) 
{
	...
	for (n = 0 ; n  < nbr_iterations ; n++){
		...
		if (prank==psize-1){
			Nblock = nbr_particles-(psize-1)*Nblock;
		}
		...
		for(int i = start; i<end; i++){
			compute_force_particle(root1,&recv_particles[i]);

		}
		particle_t * send_particles;
		send_particles  = malloc(Nblock*sizeof(particle_t));
		update_particles(send_particles,recv_particles,step,start,end);
		
	

		for(int i = 0; i<nbr_particles;i++){
			send_particles[i]=recv_particles[i+start];
		}

		particle_t * received;
		received = malloc(nbr_particles*sizeof(particle_t));




		printf("NBLOCK SIZE %d\n",Nblock);
		//MPI---------------------------------------------------------------------------------------------------
		for(int i = 0; i<Nblock; i++){
			printf("rank %d old part id %d\n",prank,send_particles[i].id);
		}

		MPI_Allgather(send_particles,Nblock,particle_type,received,Nblock,particle_type,MPI_COMM_WORLD);
		for(int i = 0; i<10; i++){
			printf("rank %d new part id %d\n",prank,received[i].id);
		}
		//-----------------------------------------------------------------------------------------------------
...
void update_particles(particle_t * send_particles,particle_t * new_particles,double step,int start, int end){
	for(int i = start; i<end;i++){
		double ax,ay,az;
		send_particles[i].x += new_particles[i].vx * step;	 
		send_particles[i].y += new_particles[i].vy * step;	 
		send_particles[i].z += new_particles[i].vz * step;	 
		ax = new_particles[i].fx/new_particles[i].m;
		ay = new_particles[i].fy/new_particles[i].m;
		az = new_particles[i].fz/new_particles[i].m;
		send_particles[i].vx += ax*step;
		send_particles[i].vy += ay*step;
		send_particles[i].vz += az*step;
