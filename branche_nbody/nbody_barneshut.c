...
void get_particles_from_branche(node * branche, particle_t*  new_particles, int * count_part){
		int i;
		node * tmp;

		if(branche==NULL) return;
		if(branche->children!=NULL){
			for(i=0;i<8;i++){
				tmp = &branche->children[i];
				get_particles_from_branche(tmp,new_particles,count_part);
			}
		}
		if (branche->particle!=NULL){
			new_particles[*count_part]=*(branche->particle);
			(*count_part)++;
...
void nbodybarneshut (particle_t * array, int nbr_particles, int nbr_iterations, int psize, int prank) 
{
	...
	particle_t tmp;
	node * branche;
	particle_t * new_particles; //we create an array of particles with computed forces
	...
	int gen = 1;
	if (psize>8) gen =2;
	int Nblock=pow(8,gen)/psize;	

	if (prank==psize-1){
		Nblock = pow(8,gen)-(psize-1)*Nblock;
	}

	root1 = malloc(sizeof(node));	
	branche = malloc(sizeof(node));	
	tmp = getMinMax(array, nbr_particles);
	init_tree(&tmp, root1);	
	construct_bh_tree(array,root1,nbr_particles);
	
	int start = prank*Nblock;
	int end = (prank+1)*Nblock;
	
	for (n = 0 ; n  < nbr_iterations ; n++){
		new_particles =  malloc(nbr_particles*sizeof(particle_t));
		nbr_particles = root1->sub_nbr_particles;
		int particles_per_processes =0; //get the number of particles per processes
		int current_size = 0 ;		//current size of the new_particles array
		for(int id = start; id < end; id++){
			
			if (gen==1){
				branche = &root1->children[id];
			}else{
				node * child_tmp;
				child_tmp = malloc(sizeof(node));
				child_tmp = &root1->children[id/8];
				if(child_tmp!=NULL){
				branche = &child_tmp->children[id%8];
				}
				else{
					branche = NULL;
				}
			...
			if((branche->sub_nbr_particles>0)&&(branche!=NULL)){
			compute_force_in_node(branche, root1);
			get_particles_from_branche(branche,new_particles,&current_size);
			
			particles_per_processes+=branche->sub_nbr_particles;
			}
		}
		
		
		int receive_count =root1->sub_nbr_particles;
		particle_t * recv_particles;
		recv_particles = malloc(receive_count*sizeof(particle_t));
		if(particles_per_processes>0) update_particles(new_particles,step,particles_per_processes);

		int displacement_tmp[psize];
		int displacement[psize];
		MPI_Allgather(&particles_per_processes,1,MPI_INT,displacement_tmp,1,MPI_INT,MPI_COMM_WORLD);

		for(int i = 0; i<psize;i++){
			int tmp = 0;
			for(int j = 0; j<i;j++){
				tmp+=displacement_tmp[j];
			}
			displacement[i]=tmp;
		}
		displacement[0]=0;
		
		MPI_Allgatherv(new_particles,particles_per_processes,particle_type,recv_particles,displacement_tmp,displacement,particle_type, MPI_COMM_WORLD);
		clean_tree(root1);
...
void compute_force_particle(node * n, particle_t * p){
	...
	if ((n->particle != NULL)&&(n->children==NULL)) {
		//printf("WE COMPUTE THE FORCE OF THE PARTICLE\n");
		//print_particle(p);
...
void construct_bh_tree(particle_t * array, node *  root, int nbr_particles){
	...
	for (i=0;i < nbr_particles; i++){
		if (! is_particle_out_of_scope(&array[i],root)) {
